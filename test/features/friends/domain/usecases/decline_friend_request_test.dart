...
void main() {
  ...
  setUp(() {
    ...
    usecase = DeclineFriendRequest(mockFriendRepository);
  });

  const tFriendshipId = 'friendship-123';

  test('should return void when declineFriendRequest is successful', () async {
    // arrange
    when(() => mockFriendRepository.declineFriendRequest(tFriendshipId))
    ...
    verify(() => mockFriendRepository.declineFriendRequest(tFriendshipId)).called(1);
    verifyNoMoreInteractions(mockFriendRepository);
  });

  test('should return ServerFailure when declineFriendRequest fails', () async {
    // arrange
    when(() => mockFriendRepository.declineFriendRequest(tFriendshipId))
        .thenAnswer((_) async => const Left(ServerFailure(message: 'Failed to decline request')));

    // act
    final result = await usecase(tFriendshipId);

    // assert
    expect(result, const Left(ServerFailure(message: 'Failed to decline request')));
    verify(() => mockFriendRepository.declineFriendRequest(tFriendshipId)).called(1);
    verifyNoMoreInteractions(mockFriendRepository);
  });
